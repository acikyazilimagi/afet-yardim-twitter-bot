...
func (s botService) Retweet(ctx context.Context, req structure.RetweetRequest) (structure.RetweetResponse, error) {
	mLogger := s.logger.WithFields(logrus.Fields{
		"method":  "Retweet",
		"tweetId": req.TweetID,
	})

	_, _, err := s.twitterClient.Statuses.Retweet(req.TweetID, nil)
	if err != nil {
	...
	return structure.RetweetResponse{IsSuccess: true}, nil
}
