...
func Run() {
	...
	server := initHTTPHandler(endpoints)
...
func initHTTPHandler(endpoints endpoint.Endpoints) *http.Server {
	...
	server := &http.Server{
		...
		Handler:      handler.NewHTTPHandler(endpoints),
	}
...
func initConfig() {
	...
	if err == nil {
		log.Println("configs are loaded from environment. no need to load .env file")
		return
	}
